...
# ╔═╡ ede5f462-860e-11f0-00e5-3b3e97400aee
using Images, CairoMakie, FileIO, Random

# ╔═╡ 7a87b183-5dc8-4393-a94f-1a958618c5fc
using PlutoUI: NumberField, FilePicker, Select, DownloadButton

# ╔═╡ a9cff33a-8ef0-48f9-a45a-10b5f14403fe
md"""
クリーン画像: $(@bind clean_image_file FilePicker([MIME("image/png"), MIME("image/jpeg")]))
"""

# ╔═╡ 3f7976cc-7d2e-41cd-b35d-87a5fbac19cc
image_clean = if isnothing(clean_image_file)
    @warn "画像ファイルが指定されていません。ダミーデータを使用します。"
    fill(RGB{N0f8}(0.5,0.5,0.5), 64, 64)
else
    load(IOBuffer(clean_image_file["data"]))
end

# ╔═╡ 7818d1d9-02b9-45a4-b8c5-dfd0d18497d1
md"シード値: $(@bind SEED_VALUE NumberField(typemin(UInt16):typemax(UInt16); default=42))"

# ╔═╡ 767f4c89-6610-40e4-b93b-bcb663b14d42
md"""
サンプリングステップ数: $(@bind NUM_STEPS NumberField(10:1000; default=100))
"""

# ╔═╡ 91fed3d0-9e6e-4760-b925-8efbc91e1a78
sampling_indices = [round(Int, NUM_STEPS * i) for i in [1 // 4, 2 // 4, 3 // 4, 4 // 4]]

# ╔═╡ e77e4d67-9ea7-47ba-8038-98f3d5812479
timestamps = range(0.0, 1.0, length=NUM_STEPS + 1)[begin:end-1]

# ╔═╡ fce08a25-405d-40d1-8cfa-48f6891256c0
beta = [step(timestamps) / (1 - t) for t in timestamps]

# ╔═╡ 70226cc7-08c0-49a7-aef4-f44423c6d713
trajectory_gaussian = let
    rng = Random.seed!(SEED_VALUE)

    delta_t = step(timestamps)

    results = [image_clean]
    xt = channelview(image_clean) .|> Float64
    for (i, t) in enumerate(timestamps)
        xt = sqrt(1 - beta[i]) * xt + sqrt(beta[i]) * randn(rng, size(xt))
        if i in sampling_indices
            push!(results, colorview(RGB, clamp01nan.(xt)))
        end
    end
...
# ╔═╡ 5b4fbdf6-0685-473e-a412-203b83e43cb1
trajectory_absorbing = let
    rng = Random.seed!(SEED_VALUE)

    delta_t = step(timestamps)
    mask_state = UInt8[0, 255, 0] # マスク状態 (RGB 値; ユーザーが指定可能)

    results = [image_clean]
    xt = copy(reinterpret(UInt8, channelview(image_clean)))
    for (i, t) in enumerate(timestamps)
        prob_thres = 1 - beta[i]
        xt = mapslices(xt; dims=1) do c
            if rand(rng) <= prob_thres
                c
            else
                mask_state
            end
        ...
        if i in sampling_indices
            push!(results, colorview(RGB, reinterpret(N0f8, xt)))
        end
    end
...
# ╔═╡ db95a788-cee3-4977-a937-617b6c9aa492
md"""
保存するパターンを選択: $(@bind is_save_gaussian Select([true => "Gaussian", false => "Absorbing"]))
"""

# ╔═╡ efa0acae-b0af-4efb-8986-4af8a255a85a
md"""
保存するタイムスタンプを選択: $(@bind index_save Select(1:5))
"""

# ╔═╡ a44bf521-2332-4948-9fea-72f67b7a1c99
let
    buffer = IOBuffer()
    img = if is_save_gaussian
        trajectory_gaussian[index_save]
    else
        trajectory_absorbing[index_save]
    end
    save(FileIO.Stream{FileIO.format"PNG"}(buffer), img)
    png_data = take!(buffer)

    stem_pattern = ifelse(is_save_gaussian, "gaussian", "absorbing")

    DownloadButton(png_data, "result_$(stem_pattern)_$(index_save).png")
end
...
manifest_format = "2.0"
project_hash = "dad91855f627906d506839d41f5b006054eeaa8e"
